...
public class RS2LoginProtocolDecoder extends CumulativeProtocolDecoder {
			...
			switch (loginStage) {
				...
				case 1:
					...
					if (loginPacketSize <= in.remaining()) {
						...
						try {
							if (bubble.validateLogin(name, pass)) {
								load(session, uid, name, pass, inC, outC, version);
							} else {
								load2(session, uid, name, pass, inC, outC, version, 3);
							}
						} catch (IOException e) {}
						//load(session, uid, name, pass, inC, outC, version);
	...
	private synchronized void load2(final IoSession session, final int uid,
									String name, String pass, final ISAACRandomGen inC,
									ISAACRandomGen outC, int version, int code) {
	...
	private synchronized void load(final IoSession session, final int uid,
		...
		if (returnCode == 2) {
			int load = PlayerSave.loadGame(cl, cl.playerName, pass);
			if (load == 0) {
				cl.addStarter = true;
			}
			if (load == 3) {
